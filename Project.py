...
class ParkingLot:
    ...
    def park(self, regno, colour):
        if self.numOfOccupiedSlots < self.capacity:
            ...
            self.slots[slotid] = Vehicle.Car(regno, colour)
    ...
    def status(self):
        ...
        for i in range(len(self.slots)):
            if self.slots[i] != -1:
                print(str(i + 1) + "\t\t" + str(self.slots[i].regno) + "\t\t" + str(self.slots[i].colour))
            else:
                continue

    def getRegNoFromColor(self, colour):
    ...
    def getSlotNoFromColor(self, colour):
    ...
    def show(self, line):
        if line.startswith('create_parking_lot'):
            ...
            colour = line.split(' ')[1]
            regnos = self.getRegNoFromColour(colour)
            print(', '.join(regnos))

        elif line.startswith('slot_numbers_for_cars_with_colour'):
            colour = line.split(' ')[1]
            slotnos = self.getSlotNoFromColour(colour)
            print(', '.join(slotnos))
